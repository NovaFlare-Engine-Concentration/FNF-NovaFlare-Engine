...
class Save implements ConsoleClass
{
  ...
  public static function getDefaultData():RawSaveData
  {
    ...
    return {
      ...
      mobileOptions: {
        // Reasonable defaults.
        screenTimeout: false,
        controlsScheme: FunkinHitboxControlSchemes.Origin,
        controlPositions: [],
        noAds: false
      },
      #end
  ...
  public function serializeJson(pretty:Bool = true):String
  {
    ...
    return funkin.util.SerializerUtil.toJSON(data, pretty);
    #else
...
typedef SaveDataOptions =
{
  ...
  /**
   * If enabled, center the player's strumline.
   * @default `false`
   */
   ...
   * @param fancyPreview Show a fancy preview? Default: `false`
...
typedef SaveDataMobileOptions =
{
  ...
  /**
   * User-defined x/y coordinates for Pad-Custom's four buttons.
   */
  var controlPositions:Array<Array<Float>>;
