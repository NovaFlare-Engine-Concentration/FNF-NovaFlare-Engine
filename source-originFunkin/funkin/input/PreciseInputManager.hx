...
class PreciseInputManager extends FlxKeyManager<FlxKey, PreciseInputList>
{
  ...
  var _boundGamepads:Map<Int, FlxGamepad>;
  ...
  public function new()
  {
    ...
    FlxG.stage.addEventListener(KeyboardEvent.KEY_DOWN, handleOpenFLKeyDown);
    FlxG.stage.addEventListener(KeyboardEvent.KEY_UP, handleOpenFLKeyUp);
    FlxG.signals.preUpdate.add(pollGamepads);
  ...
  public function initializeButtons(controls:Controls, gamepad:FlxGamepad):Void
  {
    ...
    _boundGamepads.set(gamepad.id, gamepad);
  ...
  /**
   * NovaFlare's current Lime does not expose the upstream FNF
   * `onKey*Precise` hooks. OpenFL still delivers the native event immediately,
   * so timestamp it at the same boundary and keep the original dispatch API.
   */
  function handleOpenFLKeyDown(event:KeyboardEvent):Void
  {
    var key:FlxKey = cast event.keyCode;
    if (_keyList.indexOf(key) == -1) return;
  ...
  function handleOpenFLKeyUp(event:KeyboardEvent):Void
  {
    var key:FlxKey = cast event.keyCode;
    if (_keyList.indexOf(key) == -1) return;
  ...
  /**
   * Gamepad transitions are already timestamped by Lime before Flixel's
   * pre-update signal. Polling at this boundary preserves just-pressed and
   * just-released events without depending on FNF's custom Lime fork.
   */
  function pollGamepads():Void
  {
    var timestamp:Int64 = getCurrentTimestamp();
    for (gamepadId in _boundGamepads.keys())
    {
      var gamepad:FlxGamepad = _boundGamepads.get(gamepadId);
      var buttons:Array<FlxGamepadInputID> = _buttonList.get(gamepadId);
      if (gamepad == null || buttons == null) continue;

      for (button in buttons)
      {
        var direction:NoteDirection = getDirectionForButton(gamepad, button);
        if (gamepad.checkStatus(button, JUST_PRESSED))
        {
          onInputPressed.dispatch({noteDirection: direction, timestamp: timestamp, keyCode: cast button});
          _dirPressTimestamps.set(direction, timestamp);
        }
        if (gamepad.checkStatus(button, JUST_RELEASED))
        {
          onInputReleased.dispatch({noteDirection: direction, timestamp: timestamp, keyCode: cast button});
  ...
  function clearButtons():Void
  {
    ...
    _boundGamepads.clear();
  }

  public override function destroy():Void
  {
    // Keyboard
    FlxG.stage.removeEventListener(KeyboardEvent.KEY_DOWN, handleOpenFLKeyDown);
    FlxG.stage.removeEventListener(KeyboardEvent.KEY_UP, handleOpenFLKeyUp);
    FlxG.signals.preUpdate.remove(pollGamepads);
