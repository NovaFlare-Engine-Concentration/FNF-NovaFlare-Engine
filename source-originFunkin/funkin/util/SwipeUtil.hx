...
class SwipeUtil
{
  ...
  {
    #if FEATURE_TOUCH_CONTROLS
    final swipe:Null<FlxSwipe> = getCurrentFlick();
    return swipe != null && swipe.degrees > 45 && swipe.degrees < 135;
    #else
    return false;
    #end
  }
  ...
  inline static function get_flickRight():Bool
  {
    #if FEATURE_TOUCH_CONTROLS
    final swipe:Null<FlxSwipe> = getCurrentFlick();
    return swipe != null && swipe.degrees > -45 && swipe.degrees < 45;
    #else
    return false;
    #end
  }
  ...
  inline static function get_flickLeft():Bool
  {
    #if FEATURE_TOUCH_CONTROLS
    final swipe:Null<FlxSwipe> = getCurrentFlick();
    return swipe != null && (swipe.degrees > 135 || swipe.degrees < -135);
    #else
    return false;
    #end
  }
  ...
  inline static function get_flickDown():Bool
  {
    #if FEATURE_TOUCH_CONTROLS
    final swipe:Null<FlxSwipe> = getCurrentFlick();
    return swipe != null && swipe.degrees > -135 && swipe.degrees < -45;
    #else
    return false;
    #end
  }
  ...
  {
    // NF's selected Flixel exposes completed FlxSwipe values instead of the
    // upstream FNF fork's persistent flickManager. The global swipe list is
    // cleared by FlxGame at the end of the current frame.
  }

  #if FEATURE_TOUCH_CONTROLS
  static inline function getCurrentFlick():Null<FlxSwipe>
  {
    if (FlxG.swipes.length == 0) return null;
    var swipe:FlxSwipe = FlxG.swipes[FlxG.swipes.length - 1];
    return swipe.distance >= 60 && swipe.duration <= 0.3 ? swipe : null;
  }
  #end
}
