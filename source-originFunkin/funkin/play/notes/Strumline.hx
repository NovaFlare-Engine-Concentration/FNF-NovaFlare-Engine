...
class Strumline extends FlxSpriteGroup
{
  ...
  /**
   * Every black panel currently used by the strumline. Normally this only
   * contains `background`; middle scroll replaces it with one panel for each
   * separated receptor block.
   */
  ...
  /**
   * Per-direction horizontal offsets used by middle scroll to split the
   * opponent receptors while keeping notes, sustains and effects aligned.
   */
  var directionXOffsets:Array<Float> = [0.0, 0.0, 0.0, 0.0];

  /**
   * Base opacity for receptors and incoming notes on this strumline.
   */
  public var visualAlpha:Float = 1.0;
  ...
  override function set_y(value:Float):Float
  {
    ...
    {
      for (panel in this.backgrounds)
      {
        if (panel != null) panel.y = 0;
      }
    }
  ...
  override function set_alpha(value:Float):Float
  {
    ...
    if (this.backgrounds != null)
    {
      for (panel in this.backgrounds)
      {
        if (panel != null) panel.alpha = Preferences.strumlineBackgroundOpacity / 100.0 * alpha;
      }
    }
  ...
  /**
   * Move the four directions to absolute screen X positions.
   */
  public function setDirectionScreenPositions(screenPositions:Array<Float>):Void
  {
    if (screenPositions == null || screenPositions.length < KEY_COUNT) return;

    for (i in 0...KEY_COUNT)
    {
      directionXOffsets[i] = 0.0;
    }
    setNoteSpacing(noteSpacingScale);

    for (i in 0...KEY_COUNT)
    {
      var receptor:StrumlineNote = getByDirection(DIRECTIONS[i]);
      directionXOffsets[i] = screenPositions[i] - receptor.x;
    }
    setNoteSpacing(noteSpacingScale);
  }

  /**
   * Split the black strumline background around separated receptor groups.
   * Direction positions are already in screen coordinates, so each generated
   * panel is placed behind the exact receptor bounds.
   */
  public function setBackgroundDirectionGroups(directionGroups:Array<Array<NoteDirection>>):Void
  {
    if (directionGroups == null || directionGroups.length == 0) return;

    for (panel in backgrounds)
    {
    ...
    for (directions in directionGroups)
    {
      ...
      {
        final receptor:StrumlineNote = getByDirection(direction);
        if (receptor == null) continue;

        left = Math.min(left, receptor.x);
        right = Math.max(right, receptor.x + receptor.width);
      }

      if (!Math.isFinite(left) || !Math.isFinite(right) || right <= left) continue;

      final panel:FunkinSprite = new FunkinSprite(left - this.x - BACKGROUND_PAD, -this.y)
        .makeSolidColor(Std.int(Math.ceil(right - left + BACKGROUND_PAD * 2)), FlxG.height, 0xFF000000);
      panel.alpha = Preferences.strumlineBackgroundOpacity / 100.0;
      panel.scrollFactor.set(0, 0);
      add(panel);
      backgrounds.push(panel);
      if (background == null) background = panel;
    }
  ...
  public function buildHoldNoteSprite(note:SongNoteData):SustainTrail
  {
    ...
    if (holdNoteSprite != null)
    {
      ...
      holdNoteSprite.alpha = visualAlpha;
  ...
  function getXPos(direction:NoteDirection):Float
  {
    ...
    #end
    var basePosition:Float = switch (direction)
    {
    ...
    var directionIndex:Int = DIRECTIONS.indexOf(direction);
    return basePosition + (directionIndex >= 0 ? directionXOffsets[directionIndex] : 0.0);
  }
  ...
  function fadeInArrow(index:Int, arrow:StrumlineNote):Void
  {
    arrow.y -= 10;
    arrow.alpha = 0.0;
    FlxTween.tween(arrow, {y: arrow.y + 10, alpha: visualAlpha}, 1, {ease: FlxEase.circOut, startDelay: 0.5 + (0.2 * index)});
  }
  ...
  override function findMinYHelper():Float
  {
    ...
    for (member in group.members)
    {
      ...
      // SKIP THE BACKGROUND
      if (backgrounds != null && backgrounds.indexOf(cast member) != -1) continue;
