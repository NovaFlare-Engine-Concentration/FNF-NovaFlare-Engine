...
class Stage extends FlxSpriteGroup implements IPlayStateScriptedClass implements IRegistryEntry<StageData>
{
  ...
  public function addCharacter(character:BaseCharacter, charType:CharacterType):Void
  {
    ...
    cacheCharacterTexture(character);
  ...
  /**
   * Keep a character's primary bitmap alive for the current song.
   *
   * Mod scripts may destroy and recreate characters during a song retry. Without
   * registering the bitmap with FunkinMemory, destroying the old character can
   * evict its graphic and force the replacement to decode the full atlas again.
   */
  function cacheCharacterTexture(character:BaseCharacter):Void
  {
    var characterData = CharacterDataParser.fetchCharacterData(character.characterId);
    if (characterData == null || characterData.assetPath == null || characterData.assetPath.length == 0) return;

    switch (characterData.renderType)
    {
      case CharacterRenderType.Sparrow | CharacterRenderType.Packer | CharacterRenderType.MultiSparrow:
        FunkinMemory.cacheTexture(Paths.image(characterData.assetPath));
      default:
        // Animate Atlas and custom characters do not use a single image asset key.
    }
  }
