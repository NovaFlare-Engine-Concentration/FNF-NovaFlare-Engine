...
class PlayState extends MusicBeatSubState
{
  ...
  **/
  ...
  function initStrumlines():Void
  {
    ...
    if (Preferences.usesMiddleScroll())
    {
      final leftReceptor = playerStrumline.getByDirection(NoteDirection.LEFT);
      final rightReceptor = playerStrumline.getByDirection(NoteDirection.RIGHT);
      final receptorCenter:Float = (leftReceptor.x + rightReceptor.x + rightReceptor.width) * 0.5;
      playerStrumline.x += FullScreenScaleMode.getSafeCenterX() - receptorCenter;

      final safeLeft:Float = FullScreenScaleMode.getSafeLeft();
      final safeRight:Float = FullScreenScaleMode.getSafeRight();
      final safeWidth:Float = safeRight - safeLeft;
      final edgePadding:Float = Math.max(24.0, safeWidth * 0.075);
      final spacing:Float = Strumline.NOTE_SPACING;
      opponentStrumline.visualAlpha = 0.35;
      opponentStrumline.setDirectionScreenPositions([
        safeLeft + edgePadding,
        safeLeft + edgePadding + spacing,
        safeRight - edgePadding - spacing * 2,
        safeRight - edgePadding - spacing
      ]);
      opponentStrumline.setBackgroundDirectionGroups([
        [NoteDirection.LEFT, NoteDirection.DOWN],
  ...
  function initNoteHitbox()
  {
    ...
    // Center the actually rendered receptors, rather than Strumline.width.
    // Strumline.width contains a trailing spacing slot, and the mobile code
    // above doubles each receptor's input width without doubling its graphic.
    final leftReceptor = playerStrumline.getByDirection(NoteDirection.LEFT);
    final rightReceptor = playerStrumline.getByDirection(NoteDirection.RIGHT);
    final renderedReceptorWidth:Float = rightReceptor.width * 0.5;
    final receptorCenter:Float = (leftReceptor.x + rightReceptor.x + renderedReceptorWidth) * 0.5;
    playerStrumline.x += FullScreenScaleMode.getSafeCenterX() - receptorCenter;
  ...
  function processInputQueue():Void
  {
    ...
    while (inputPressQueue.length > 0)
    {
      ...
      var shouldGhostMiss:Bool = !Preferences.ghostTapping;
      #if FEATURE_GHOST_TAPPING
      shouldGhostMiss = shouldGhostMiss && !playerStrumline.mayGhostTap();
      #end
      if (shouldGhostMiss && notesInDirection.length == 0 && !isAlreadyHeld)
      {
