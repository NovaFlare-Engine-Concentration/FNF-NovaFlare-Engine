...
{
  /**
   * Some base-game scripts import library classes that no compiled FNF class
   * references directly. Keep them in the executable so Polymod can resolve
   * those imports at runtime without changing NovaFlare's selected haxelibs.
   */
  @:keep
  static final ORIGIN_CORE_SCRIPT_DEPENDENCIES:Array<Class<Dynamic>> = [
    animate.internal.elements.FlxSpriteElement,
    flixel.addons.effects.FlxSkewedSprite,
    funkin.audio.visualize.ABotVis,
    funkin.effects.FunkTrail,
    funkin.graphics.shaders.AdjustColorShader,
    funkin.graphics.shaders.DropShadowShader,
    funkin.graphics.shaders.OverlayBlend,
    funkin.graphics.shaders.RuntimeRainShader,
    funkin.graphics.shaders.SserafimShader,
    funkin.graphics.shaders.TextureSwap,
    funkin.graphics.shaders.WiggleEffectRuntime,
    funkin.util.FlxTweenUtil
  ];
  ...
  static function getModFolder():String
  {
    ...
    {
      return originfunkin.OriginFunkinMode.getModRoot();
    }
    return MOD_FOLDER;
  }
