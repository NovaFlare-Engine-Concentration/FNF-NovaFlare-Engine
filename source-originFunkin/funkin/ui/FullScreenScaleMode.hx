...
class FullScreenScaleMode extends flixel.system.scaleModes.BaseScaleMode
{
  ...
  /**
   * Left edge of the horizontally usable game area, in Flixel coordinates.
   *
   * `FlxG.width` already includes the extra logical width supplied by this
   * scale mode. Only an edge notch needs to shrink that logical area.
   */
  public static function getSafeLeft():Float
  {
    if (gameNotchSize.x <= 0) return 0;

    final notchCenter:Float = gameNotchPosition.x + gameNotchSize.x * 0.5;
    return notchCenter < FlxG.width * 0.5 ? Math.max(0, gameNotchPosition.x + gameNotchSize.x) : 0;
  }

  /**
   * Right edge of the horizontally usable game area, in Flixel coordinates.
   */
  public static function getSafeRight():Float
  {
    if (gameNotchSize.x <= 0) return FlxG.width;

    final notchCenter:Float = gameNotchPosition.x + gameNotchSize.x * 0.5;
    return notchCenter > FlxG.width * 0.5 ? Math.min(FlxG.width, gameNotchPosition.x) : FlxG.width;
  }

  /**
   * Horizontal center of the usable game area.
   */
  public static function getSafeCenterX():Float
  {
    return (getSafeLeft() + getSafeRight()) * 0.5;
  }
