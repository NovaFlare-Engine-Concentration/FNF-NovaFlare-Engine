...
class PreferencesMenu extends Page<OptionsState.OptionsMenuPageName>
{
  ...
  {
    #if mobile
    createPrefItemEnum('Mobile Controls', 'Choose between the original arrow controls and the full-screen hitbox.',
      [
        "Origin" => FunkinHitboxControlSchemes.Origin,
        "Hitbox" => FunkinHitboxControlSchemes.Hitbox
      ],
      function(key:String, value:String):Void
      {
        Preferences.controlsScheme = value;
      },
      Preferences.usesOriginControls() ? "Origin" : "Hitbox");
    #end
    ...
    // VSync is only exposed on desktop.
    #if !(mobile || web)
    createPrefItemEnum('VSync', "When enabled, the game attempts to match the framerate with your monitor's refresh rate.",
      ["Off" => OriginVSyncMode.OFF, "On" => OriginVSyncMode.ON, "Adaptive" => OriginVSyncMode.ADAPTIVE,],
      function(key:String, value:OriginVSyncMode):Void
    {
      trace("Setting vsync mode to " + key);
      Preferences.vsyncMode = value;
    }, switch (Preferences.vsyncMode)
      {
        case OriginVSyncMode.OFF: "Off";
        case OriginVSyncMode.ON: "On";
        case OriginVSyncMode.ADAPTIVE: "Adaptive";
      });
    #end
