...
class FunkinButton extends FunkinSprite implements IFlxInput
{
  ...
  public override function update(elapsed:Float):Void
  {
    ...
    if (visible)
    {
      ...
      {
        final trackedTouch:Null<FlxTouch> = currentTouch;

        // Android may cancel an entire pointer stream when its gesture system
        // intercepts a multi-touch sequence. Flixel then removes the touch on
        // the following frame, so explicitly release a button whose captured
        // pointer disappeared instead of leaving every lane latched.
        if (trackedTouch == null)
        {
          onOutHandler();
        }
        else if (trackedTouch.released)
        {
          if (touchOverlaps(trackedTouch))
            onUpHandler();
          else
            onOutHandler();
        }
        else if (!checkTouchOverlap(trackedTouch) && limitToBounds)
        {
          onOutHandler();
        }
      }
  ...
  function checkTouchOverlap(?touch:FlxTouch):Bool
  {
    ...
    for (candidate in touches)
    {
      ...
      // ownership churn and left the lane waiting for the wrong release event.
      if (status != FunkinButtonStatus.NORMAL && candidate.touchPointID != touchID) continue;
      if (!touchOverlaps(candidate) || !claimTouch(candidate)) continue;
  ...
  function touchOverlaps(touch:FlxTouch):Bool
  {
    for (camera in cameras)
    {
      ...
      {
        if (polygonOverlapsPoint(worldPos, false, camera)) return true;
      }
      else if (radius > 0)
      {
        if (circleOverlapsPoint(worldPos, camera)) return true;
      }
      else if (overlapsPoint(worldPos, true, camera))
      {
        return true;
      }
    }
  ...
  function claimTouch(touch:FlxTouch):Bool
  {
    final id:Int = touch.touchPointID;
    final previousOwner:Null<FunkinButton> = buttonsTouchID.get(id);

    if (previousOwner != null && previousOwner != this)
    {
      // Only swipe-style buttons deliberately transfer a held pointer.
      if (!previousOwner.limitToBounds && previousOwner.input != null)
        previousOwner.onOutHandler();
      else
        return false;
    }

    touchID = id;
    buttonsTouchID.set(id, this);
    updateStatus(touch);
    return true;
  }

  inline function releaseTouchOwnership():Void
  {
    if (touchID >= 0 && buttonsTouchID.get(touchID) == this)
