...
class FunkinHitbox extends FlxTypedSpriteGroup<FunkinHint>
{
  ...
  public function new(?schemeOverride:String, ?showGradint:Bool = true, ?directionsOverride:Array<NoteDirection>, ?colorsOverride:Array<FlxColor>):Void
  {
    ...
    {
      case FunkinHitboxControlSchemes.Hitbox | FunkinHitboxControlSchemes.FourLanes:
      ...
      case FunkinHitboxControlSchemes.PadRight:
        addDesktopPad(FullScreenScaleMode.getSafeRight() - 245, FlxG.height - 235, hintsNoteDirections);
      case FunkinHitboxControlSchemes.PadLeft:
        addDesktopPad(FullScreenScaleMode.getSafeLeft() + 245, FlxG.height - 235, hintsNoteDirections);
      case FunkinHitboxControlSchemes.PadCustom:
        addDesktopPad(FullScreenScaleMode.getSafeRight() - 245, FlxG.height - 235, hintsNoteDirections);

        final savedPositions:Array<Array<Float>> = Preferences.mobileControlPositions;
        for (i in 0...members.length)
        {
          if (i < savedPositions.length && savedPositions[i] != null && savedPositions[i].length >= 2)
          {
            members[i].setPosition(savedPositions[i][0], savedPositions[i][1]);
          }
        }
      case FunkinHitboxControlSchemes.PadDuo:
        addDesktopPad(FullScreenScaleMode.getSafeLeft() + 245, FlxG.height - 235, hintsNoteDirections);
        addDesktopPad(FullScreenScaleMode.getSafeRight() - 245, FlxG.height - 235, hintsNoteDirections);
      case FunkinHitboxControlSchemes.Origin | FunkinHitboxControlSchemes.Arrows:
        final hintWidth:Int = 146;
        final hintHeight:Int = 149;
        final noteSpacing:Int = 80;

        // There are only `count - 1` gaps. Counting a trailing gap shifted the
        // complete arrow row 40 pixels left, which was especially noticeable
        // after FullScreenScaleMode expanded FlxG.width on wide displays.
        final totalWidth:Int = hintWidth * hintsNoteDirections.length + noteSpacing * (hintsNoteDirections.length - 1);
        final xPos:Int = Math.floor(FullScreenScaleMode.getSafeCenterX() - totalWidth * 0.5);
      ...
      case FunkinHitboxControlSchemes.Keyboard:
        // No touch buttons. Physical keyboard/gamepad input remains active.
    }
    #end
  ...
  /**
   * Creates a diamond pad using the same receptor art and press animations as
   * desktop gameplay.
   */
  function addDesktopPad(centerX:Float, centerY:Float, directions:Array<NoteDirection>):Void
  {
    final hintSize:Int = 112;
    final axisOffset:Int = 105;
    final positions:Array<Array<Float>> = [
      [centerX - axisOffset - hintSize / 2, centerY - hintSize / 2],
      [centerX - hintSize / 2, centerY + axisOffset - hintSize / 2],
      [centerX - hintSize / 2, centerY - axisOffset - hintSize / 2],
      [centerX + axisOffset - hintSize / 2, centerY - hintSize / 2]
    ];
    ...
    {
      final hint:FunkinHint = createHintTransparentNote(positions[i][0], positions[i][1], directions[i], hintSize, hintSize);
      hint.alpha = 0.65;
      add(hint);
    }
  }
