 ...
 */
class NoteSplashEditorMenuBar extends FlxSpriteGroup
{
	...
	// ============ 菜单定义 ============
	public var menus:Array<NoteSplashMenuDef> = [];
	...
	var dropRows:Array<NoteSplashDropdownRow> = [];
	...
	var customControls:Array<NoteSplashCustomControl> = [];
	...
	// ============ 状态栏（独立组件） ============
	public var statusBar:NoteSplashEditorStatusBar;
	...
	var stepperOverlayCtrl:NoteSplashCustomControl = null;
	...
	var panelPressCtrl:NoteSplashCustomControl = null; // 按下时点中的展开面板
	...
	public function new()
	{
		...
		statusBar = new NoteSplashEditorStatusBar();
		...
		public function buildMenus():Void
	{
		menus = [
			{key: 'anim', isTest: false, items: [
				...
				item_label_widget('w_prefix', 'prefix', 'desc_prefix'),
				item_label_widget('w_note_data', 'note_data', 'desc_note_data'),
				item_label_widget('w_indices', 'indices', 'desc_indices'),
				item_label_widget('w_min_fps', 'min_fps', 'desc_min_fps'),
				item_label_widget('w_max_fps', 'max_fps', 'desc_max_fps'),
				SEP,
				item_cmd('anim_add', 'desc_anim_add', () -> callAction('anim_add')),
				item_cmd('anim_remove', 'desc_anim_remove', () -> callAction('anim_remove')),
			]},
			{key: 'props', isTest: false, items: [
				...
				item_widget('w_allow_rgb', 'desc_allow_rgb'),
				item_widget('w_allow_pixel', 'desc_allow_pixel'),
				SEP,
				item_cmd('template', 'desc_template', () -> callAction('template')),
				item_cmd('convert_txt', 'desc_convert_txt', () -> callAction('convert_txt')),
			]},
			{key: 'shader', isTest: false, items: [
				item_label_widget('w_target', 'target', 'desc_target'),
				item_label_widget('w_r', 'r', 'desc_r'),
				item_label_widget('w_g', 'g', 'desc_g'),
				item_label_widget('w_b', 'b', 'desc_b'),
				item_widget('w_no_replace', 'desc_no_replace'),
				SEP,
				item_cmd('reset_rgb', 'desc_reset_rgb', () -> callAction('reset_rgb')),
			]},
			...
			{key: 'help', isTest: false, items: [
				item_text('help_click', 'desc_help_click'),
				item_text('help_offset', 'desc_help_offset'),
				item_text('help_shift', 'desc_help_shift'),
				item_text('help_copy', 'desc_help_copy'),
				item_text('help_fps', 'desc_help_fps'),
...
static var SEP:NoteSplashMenuItemDef = {type: Sep, labelKey: null, descKey: null, getChecked: null, onCheck: null, onClick: null};

	function item_cmd(labelKey:String, descKey:String, onClick:Void->Void):NoteSplashMenuItemDef
	{
		return {type: Cmd, labelKey: labelKey, descKey: descKey, getChecked: null, onCheck: null, onClick: onClick};
	}

	function item_widget(widgetKey:String, descKey:String):NoteSplashMenuItemDef
	{
		return {type: Widget, labelKey: widgetKey, descKey: descKey, getChecked: null, onCheck: null, onClick: null, widgetKey: widgetKey};
	}

	function item_label_widget(widgetKey:String, labelKey:String, descKey:String):NoteSplashMenuItemDef
	{
		return {type: LabelWidget, labelKey: labelKey, descKey: descKey, getChecked: null, onCheck: null, onClick: null, widgetKey: widgetKey};
	}

	function item_text(textKey:String, descKey:String):NoteSplashMenuItemDef
	{
		return {type: TextLine, labelKey: textKey, descKey: descKey, getChecked: null, onCheck: null, onClick: null};
	}

	function item_dyn_text(dynKey:String, descKey:String):NoteSplashMenuItemDef
	{
		return {type: DynText, labelKey: dynKey, descKey: descKey, getChecked: null, onCheck: null, onClick: null};
	}

	// ============ 翻译 key 转换 ============
	// ★ dev 模式下 Language.get 找不到 key 会返回 'key (404)'（与 key 不同），
	//   所以 `translated == key` 条件失效、返回 (404) 字面量。
	//   必须在比较里同时检查 indexOf('(404)')，否则 (404) 会冒到 UI 上。
	...
	function translateItemKey(key:String):String
	{
		...
		var translated = Language.get(itemKey, 'notesplash');
		if (translated == itemKey || translated.indexOf('(404)') != -1) {
			// 尝试不带 item_ 前缀
			translated = Language.get(key.substring(2), 'notesplash');
		}
		return translated;
	}
	...
	{
		var translated = Language.get(key, 'notesplash');
		if (translated == key || translated.indexOf('(404)') != -1) {
			// 尝试带 label_ 前缀
			translated = Language.get('label_' + key, 'notesplash');
		}
	...
	public function applyLang():Void
	{
		...
		for (i in 0...menus.length)
		{
			...
			var m = menus[i];
			var txt:FlxText = new FlxText(i * MENU_W, 0, MENU_W, Language.get('menu_' + m.key, 'notesplash'), 14);
	...
	override function update(elapsed:Float):Void
	{
			...
			else if (activeMenu >= 0)
			{
				if (insideUI)
				{
					...
					else
					{
						...
						{
							// ③ 普通行点击
							var hitRow:NoteSplashDropdownRow = null;
							for (row in dropRows)
							{
								if (row.hit != null)
								{
							...
							if (hitRow != null)
							{
								...
								if (hitRow.def.type == LabelWidget)
									handled = handleNoteSplashCustomControlClick(hitRow);
								if (!handled)
								{
									activateRow(hitRow);
		...
		{
			var hoverRow:NoteSplashDropdownRow = null;
			for (row in dropRows)
			{
			...
			if (FlxG.mouse.wheel != 0)
			{
				for (ctrl in customControls)
				{
					...
					renderNoteSplashDropdownOptions(ctrl);
					break;
				}
			}
		}
		...
		if (panelPressCtrl != null && FlxG.mouse.pressed)
		{
			...
			if (panelDragMoved)
			{
				var ctrl:NoteSplashCustomControl = panelPressCtrl;
				var total:Int = (ctrl.options != null) ? ctrl.options.length : 0;
				var maxScroll:Int = Std.int(Math.max(0, total - MAX_DROP_OPT));
	...
	// ============ 自绘控件：显示刷新（从原生控件读数据） ============
	function refreshNoteSplashCustomControlDisplays():Void
	{
		for (ctrl in customControls)
		{
	...
	function updateStepperDisplay(ctrl:NoteSplashCustomControl):Void
	{
		var txt:String = '';
	...
	function setControlText(ctrl:NoteSplashCustomControl, txt:String):Void
	{
	...
	function updateDropdownDisplay(ctrl:NoteSplashCustomControl):Void
	{
		var lbl:String = '';
	...
	function updateInputDisplay(ctrl:NoteSplashCustomControl):Void
	{
		var txt:String = '';
	...
	function readSliderValue(ctrl:NoteSplashCustomControl):Float
	{
		var v:Float = 0;
	...
	function updateSliderDisplay(ctrl:NoteSplashCustomControl):Void
	{
	...
	function handleNoteSplashCustomControlClick(row:NoteSplashDropdownRow):Bool
	{
		var ctrl:NoteSplashCustomControl = null;
		...
		if (ctrl.ctype == 'dropdown')
		{
			if (ctrl.box != null && pointInSprite(mx, my, ctrl.box))
			{
				...
				else { closeCustomDropdowns(); ctrl.open = true; renderNoteSplashDropdownOptions(ctrl); }
				return true;
			}
			return false;
		}
	...
	function openStepperOverlay(ctrl:NoteSplashCustomControl):Void
	{
	...
	function commitStepperOverlay():Void
	{
		...
		var ctrl:NoteSplashCustomControl = stepperOverlayCtrl;
	...
	function sliderDrag(ctrl:NoteSplashCustomControl, mx:Float):Void
	{
		if (ctrl.track == null) return;
	...
	function releaseDropdownPanelClick():Void
	{
		...
		if (ctrl.panelRows != null)
		{
			for (pr in ctrl.panelRows)
			{
				...
				{
					selectNoteSplashDropdownOption(ctrl, pr.idx);
					return;
				}
			}
		}
	...
	function selectNoteSplashDropdownOption(ctrl:NoteSplashCustomControl, optIdx:Int):Void
	{
	...
	function renderNoteSplashDropdownOptions(ctrl:NoteSplashCustomControl):Void
	{
		...
		var rows:Array<NoteSplashDropdownOption> = [];
	...
	function clearDropdownPanel(ctrl:NoteSplashCustomControl):Void
	{
		if (ctrl.panelRows != null)
		{
			for (pr in ctrl.panelRows)
	...
	// ============ 自绘控件：数值 / 输入覆盖层 ============
	function stepperPlus(ctrl:NoteSplashCustomControl):Void
	{
		stepperStep(ctrl, 1);
	}

	function stepperMinus(ctrl:NoteSplashCustomControl):Void
	{
		stepperStep(ctrl, -1);
	}

	function stepperStep(ctrl:NoteSplashCustomControl, dir:Int):Void
	{
		var w:Dynamic = ctrl.widget;
		if (w == null) return;
		try
	...
	function openInputOverlay(ctrl:NoteSplashCustomControl):Void
	{
		var w:Dynamic = ctrl.widget;
		clearActiveEditWidget();
		if (stepperOverlayCtrl != null)
	...
	public function openMenu(idx:Int):Void
	{
		...
		refreshNoteSplashCustomControlDisplays();
	}
	...
	function rebuildDropdown():Void
	{
		...
		for (it in items)
		{
			...
			if (it.type == Widget || it.type == LabelWidget)
			{
				...
				{
					// 标签最小宽度
					var lbl = Language.get(it.labelKey, 'notesplash');
			...
			{
				var txt = Language.get('item_' + it.labelKey, 'notesplash');
				var lineH = 16;
				// 估算文本行数
				var charW = 7;
			...
			{
				var lbl = Language.get('item_' + it.labelKey, 'notesplash');
		...
		for (it in items)
		{
			...
			// === Cmd 行 ===
			var labelStr = Language.get('item_' + it.labelKey, 'notesplash');
	...
	function refreshCheckIcon(icon:FlxText, it:NoteSplashMenuItemDef):Void
	{
	...
	function clearNoteSplashDropdownRows():Void
	{
	...
	function activateRow(row:NoteSplashDropdownRow):Void
	{
		...
		switch (it.type)
		{
			...
			case LabelWidget:
				// 交互已由 handleNoteSplashCustomControlClick（自绘控件）接管：
	...
	// ============ 描述条 ============
	function updateDescBar(row:NoteSplashDropdownRow):Void
	{
		...
		var txt = Language.get(row.def.descKey, 'notesplash');
...
typedef NoteSplashMenuItemDef =
{
	type:NoteSplashItemType,
...
typedef NoteSplashMenuDef =
{
	key:String,
	isTest:Bool,
	items:Array<NoteSplashMenuItemDef>
};

typedef NoteSplashDropdownRow =
{
	type:NoteSplashItemType,
	labelKey:String,
	descKey:String,
	def:NoteSplashMenuItemDef,
	hit:FlxSprite,
...
typedef NoteSplashCustomControl =
{
	key:String,
	...
	?panelBg:FlxSprite,
	?panelRows:Array<NoteSplashDropdownOption>,
	?panelH:Int,
	// slider
	?track:FlxSprite,
	?fill:FlxSprite,
...
/** dropdown 展开面板的单个选项行 */
typedef NoteSplashDropdownOption =
{
	bg:FlxSprite,
	txt:FlxText,
	hit:FlxSprite,
