 ...
 */
class DialogueCharacterEditorMenuBar extends FlxSpriteGroup
{
	...
	// ============ 菜单定义 ============
	public var menus:Array<DialogueCharacterMenuDef> = [];
	...
	var dropRows:Array<DialogueCharacterDropdownRow> = [];
	...
	var customControls:Array<DialogueCharacterCustomControl> = [];
	...
	// ============ 状态栏（独立组件） ============
	public var statusBar:DialogueCharacterEditorStatusBar;
	...
	var stepperOverlayCtrl:DialogueCharacterCustomControl = null;
	...
	var panelPressCtrl:DialogueCharacterCustomControl = null; // 按下时点中的展开面板
	...
	public function new()
	{
		...
		statusBar = new DialogueCharacterEditorStatusBar();
	...
	{
		menus = [
			{key: 'type', isTest: false, items: [
				item_cmd('side_left', 'desc_side_left', () -> callAction('side_left')),
				item_cmd('side_center', 'desc_side_center', () -> callAction('side_center')),
				item_cmd('side_right', 'desc_side_right', () -> callAction('side_right')),
			]},
			{key: 'char', isTest: false, items: [
				item_label_widget('w_image', 'image', 'desc_image'),
				item_label_widget('w_pos_x', 'pos_x', 'desc_pos_x'),
				item_label_widget('w_pos_y', 'pos_y', 'desc_pos_y'),
				item_label_widget('w_scale', 'scale', 'desc_scale'),
				item_widget('w_no_aa', 'desc_no_aa'),
				SEP,
				item_cmd('reload_image', 'desc_reload_image', () -> callAction('reload_image')),
				item_cmd('load_char', 'desc_load_char', () -> callAction('load_char')),
				item_cmd('save_char', 'desc_save_char', () -> callAction('save_char')),
			]},
			{key: 'anim', isTest: false, items: [
				item_label_widget('w_anim', 'anim', 'desc_anim'),
				item_label_widget('w_anim_name', 'anim_name', 'desc_anim_name'),
				item_label_widget('w_loop_name', 'loop_name', 'desc_loop_name'),
				item_label_widget('w_idle_name', 'idle_name', 'desc_idle_name'),
				SEP,
			...
			{key: 'view', isTest: false, items: [
				item_cmd('view_reset', 'desc_view_reset', () -> callAction('view_reset')),
				item_cmd('view_bubble', 'desc_view_bubble', () -> callAction('view_bubble')),
				item_cmd('view_ghost', 'desc_view_ghost', () -> callAction('view_ghost')),
			]},
			{key: 'file', isTest: false, items: [
				item_cmd('load', 'desc_load', () -> callAction('load')),
				item_cmd('save', 'desc_save', () -> callAction('save')),
				SEP,
				item_cmd('exit', 'desc_exit', () -> callAction('exit')),
			]},
			{key: 'help', isTest: false, items: [
				item_text('help_keys', 'desc_help_keys'),
				item_text('help_camera', 'desc_help_camera'),
				item_text('help_zoom', 'desc_help_zoom'),
				item_text('help_shift', 'desc_help_shift'),
				item_text('help_space', 'desc_help_space'),
				item_text('help_f1', 'desc_help_f1'),
				item_text('help_exit', 'desc_help_exit'),
			]},
		];
	}

static var SEP:DialogueCharacterMenuItemDef = {type: Sep, labelKey: null, descKey: null, getChecked: null, onCheck: null, onClick: null};

	function item_cmd(labelKey:String, descKey:String, onClick:Void->Void):DialogueCharacterMenuItemDef
	{
		return {type: Cmd, labelKey: labelKey, descKey: descKey, getChecked: null, onCheck: null, onClick: onClick};
	}

	function item_widget(widgetKey:String, descKey:String):DialogueCharacterMenuItemDef
	{
		return {type: Widget, labelKey: widgetKey, descKey: descKey, getChecked: null, onCheck: null, onClick: null, widgetKey: widgetKey};
	}

	function item_label_widget(widgetKey:String, labelKey:String, descKey:String):DialogueCharacterMenuItemDef
	{
		return {type: LabelWidget, labelKey: labelKey, descKey: descKey, getChecked: null, onCheck: null, onClick: null, widgetKey: widgetKey};
	}

	function item_text(textKey:String, descKey:String):DialogueCharacterMenuItemDef
	{
		return {type: TextLine, labelKey: textKey, descKey: descKey, getChecked: null, onCheck: null, onClick: null};
	}

	function item_dyn_text(dynKey:String, descKey:String):DialogueCharacterMenuItemDef
	{
	...
	function translateItemKey(key:String):String
	{
		...
		var translated = Language.get(itemKey, 'dialoguechar');
		if (translated == itemKey || translated.indexOf('(404)') != -1) {
			// 尝试不带 item_ 前缀
			translated = Language.get(key.substring(2), 'dialoguechar');
		}
		return translated;
	}
	...
	{
		var translated = Language.get(key, 'dialoguechar');
		if (translated == key || translated.indexOf('(404)') != -1) {
			// 尝试带 label_ 前缀
			translated = Language.get('label_' + key, 'dialoguechar');
		}
	...
	public function applyLang():Void
	{
		...
		for (i in 0...menus.length)
		{
			...
			var m = menus[i];
			var txt:FlxText = new FlxText(i * MENU_W, 0, MENU_W, Language.get('menu_' + m.key, 'dialoguechar'), 14);
	...
	override function update(elapsed:Float):Void
	{
			...
			else if (activeMenu >= 0)
			{
				if (insideUI)
				{
					...
					else
					{
						...
						{
							// ③ 普通行点击
							var hitRow:DialogueCharacterDropdownRow = null;
							for (row in dropRows)
							{
								if (row.hit != null)
								{
							...
							if (hitRow != null)
							{
								...
								if (hitRow.def.type == LabelWidget)
									handled = handleDialogueCharacterCustomControlClick(hitRow);
								if (!handled)
								{
									activateRow(hitRow);
		...
		{
			var hoverRow:DialogueCharacterDropdownRow = null;
			for (row in dropRows)
			{
			...
			if (FlxG.mouse.wheel != 0)
			{
				for (ctrl in customControls)
				{
					...
					renderDialogueCharacterDropdownOptions(ctrl);
					break;
				}
			}
		}
		...
		if (panelPressCtrl != null && FlxG.mouse.pressed)
		{
			...
			if (panelDragMoved)
			{
				var ctrl:DialogueCharacterCustomControl = panelPressCtrl;
				var total:Int = (ctrl.options != null) ? ctrl.options.length : 0;
				var maxScroll:Int = Std.int(Math.max(0, total - MAX_DROP_OPT));
	...
	// ============ 自绘控件：显示刷新（从原生控件读数据） ============
	function refreshDialogueCharacterCustomControlDisplays():Void
	{
		for (ctrl in customControls)
		{
	...
	function updateStepperDisplay(ctrl:DialogueCharacterCustomControl):Void
	{
		var txt:String = '';
	...
	function setControlText(ctrl:DialogueCharacterCustomControl, txt:String):Void
	{
	...
	function updateDropdownDisplay(ctrl:DialogueCharacterCustomControl):Void
	{
		var lbl:String = '';
	...
	function updateInputDisplay(ctrl:DialogueCharacterCustomControl):Void
	{
		var txt:String = '';
	...
	function readSliderValue(ctrl:DialogueCharacterCustomControl):Float
	{
		var v:Float = 0;
	...
	function updateSliderDisplay(ctrl:DialogueCharacterCustomControl):Void
	{
	...
	function handleDialogueCharacterCustomControlClick(row:DialogueCharacterDropdownRow):Bool
	{
		var ctrl:DialogueCharacterCustomControl = null;
		...
		if (ctrl.ctype == 'dropdown')
		{
			if (ctrl.box != null && pointInSprite(mx, my, ctrl.box))
			{
				...
				else { closeCustomDropdowns(); ctrl.open = true; renderDialogueCharacterDropdownOptions(ctrl); }
				return true;
			}
			return false;
		}
	...
	function openStepperOverlay(ctrl:DialogueCharacterCustomControl):Void
	{
	...
	function commitStepperOverlay():Void
	{
		...
		var ctrl:DialogueCharacterCustomControl = stepperOverlayCtrl;
	...
	function sliderDrag(ctrl:DialogueCharacterCustomControl, mx:Float):Void
	{
		if (ctrl.track == null) return;
	...
	function releaseDropdownPanelClick():Void
	{
		...
		if (ctrl.panelRows != null)
		{
			for (pr in ctrl.panelRows)
			{
				...
				{
					selectDialogueCharacterDropdownOption(ctrl, pr.idx);
					return;
				}
			}
		}
	...
	function selectDialogueCharacterDropdownOption(ctrl:DialogueCharacterCustomControl, optIdx:Int):Void
	{
	...
	function renderDialogueCharacterDropdownOptions(ctrl:DialogueCharacterCustomControl):Void
	{
		...
		var rows:Array<DialogueCharacterDropdownOption> = [];
	...
	function clearDropdownPanel(ctrl:DialogueCharacterCustomControl):Void
	{
		if (ctrl.panelRows != null)
		{
			for (pr in ctrl.panelRows)
	...
	// ============ 自绘控件：数值 / 输入覆盖层 ============
	function stepperPlus(ctrl:DialogueCharacterCustomControl):Void
	{
		stepperStep(ctrl, 1);
	}

	function stepperMinus(ctrl:DialogueCharacterCustomControl):Void
	{
		stepperStep(ctrl, -1);
	}

	function stepperStep(ctrl:DialogueCharacterCustomControl, dir:Int):Void
	{
		var w:Dynamic = ctrl.widget;
		if (w == null) return;
		try
	...
	function openInputOverlay(ctrl:DialogueCharacterCustomControl):Void
	{
		var w:Dynamic = ctrl.widget;
		clearActiveEditWidget();
		if (stepperOverlayCtrl != null)
	...
	public function openMenu(idx:Int):Void
	{
		...
		refreshDialogueCharacterCustomControlDisplays();
	}
	...
	public function closeMenu():Void
	{
		...
		dropGroup.visible = false;
		clearDialogueCharacterDropdownRows();
		hideDescBar();
	}
	...
	function rebuildDropdown():Void
	{
		...
		for (it in items)
		{
			...
			if (it.type == Widget || it.type == LabelWidget)
			{
				...
				{
					// 标签最小宽度
					var lbl = Language.get(it.labelKey, 'dialoguechar');
			...
			{
				var txt = Language.get('item_' + it.labelKey, 'dialoguechar');
				var lineH = 16;
				// 估算文本行数
				var charW = 7;
			...
			{
				var lbl = Language.get('item_' + it.labelKey, 'dialoguechar');
		...
		for (it in items)
		{
			...
			// === Cmd 行 ===
			var labelStr = Language.get('item_' + it.labelKey, 'dialoguechar');
	...
	function refreshCheckIcon(icon:FlxText, it:DialogueCharacterMenuItemDef):Void
	{
	...
	function clearDialogueCharacterDropdownRows():Void
	{
	...
	function activateRow(row:DialogueCharacterDropdownRow):Void
	{
		...
		switch (it.type)
		{
			...
			case LabelWidget:
				// 交互已由 handleDialogueCharacterCustomControlClick（自绘控件）接管：
	...
	// ============ 描述条 ============
	function updateDescBar(row:DialogueCharacterDropdownRow):Void
	{
		...
		var txt = Language.get(row.def.descKey, 'dialoguechar');
...
// ============ 类型定义 ============
enum DialogueCharacterItemType
{
	Sep;
	Cmd;
...
typedef DialogueCharacterMenuItemDef =
{
	type:DialogueCharacterItemType,
...
typedef DialogueCharacterMenuDef =
{
	key:String,
	isTest:Bool,
	items:Array<DialogueCharacterMenuItemDef>
};

typedef DialogueCharacterDropdownRow =
{
	type:DialogueCharacterItemType,
	labelKey:String,
	descKey:String,
	def:DialogueCharacterMenuItemDef,
	hit:FlxSprite,
...
typedef DialogueCharacterCustomControl =
{
	key:String,
	...
	box:FlxSprite,
	text:FlxText,
	row:DialogueCharacterDropdownRow,
	// stepper
	?btnMinus:FlxSprite,
	?btnPlus:FlxSprite,
	// dropdown 展开面板
	...
	?panelBg:FlxSprite,
	?panelRows:Array<DialogueCharacterDropdownOption>,
	?panelH:Int,
	// slider
	?track:FlxSprite,
	?fill:FlxSprite,
...
/** dropdown 展开面板的单个选项行 */
typedef DialogueCharacterDropdownOption =
{
	bg:FlxSprite,
	txt:FlxText,
	hit:FlxSprite,
