 ...
 * 外观完全自绘。StageEditorState 通过 registerWidget 注册控件。
 */
class StageEditorMenuBar extends FlxSpriteGroup
{
	// ============ NovaFlare 设计系统颜色（深色模式，AARRGGBB） ============
	// —— 中性色 ——
	static final C_PAGE_BG:FlxColor = 0xFF12141A;    // 页面底层背景
	static final C_CARD:FlxColor = 0xFF1C1F28;       // 卡片面板背景
	static final C_HOVER_GEN:FlxColor = 0x147C7FFF;  // Hover 通用底色
	static final C_BORDER:FlxColor = 0x14FFFFFF;     // 分割/边框
	static final C_TEXT_MAIN:FlxColor = 0xFFF2F3F5;  // 主文字
	static final C_TEXT_SUB:FlxColor = 0xFFC9CDD4;   // 次级文字
	static final C_TEXT_HINT:FlxColor = 0xFF86909C;  // 提示文字
	// —— 一级主菜单 ——
	static final C_MENU_GRAD_A:FlxColor = 0xFF4F46E5; // 选中渐变起点(indigo)
	static final C_MENU_GRAD_B:FlxColor = 0xFF7C3AED; // 选中渐变终点(violet)
	static final C_MENU_BAR:FlxColor = 0xFFA78BFA;    // 左侧装饰条/链接/选中文字(浅紫)
	// —— 二级子菜单 ——
	static final C_SUB_HOVER:FlxColor = 0x0F8B5CF6;  // 行 hover 底色
	static final C_SUB_SEL_BG:FlxColor = 0x268B5CF6; // 行选中底色
	static final C_SUB_SEL_TEXT:FlxColor = 0xFFA78BFA; // 行选中文字
	// —— 主按钮 Primary ——
	static final C_BTN_GRAD_A:FlxColor = 0xFF6366F1; // Primary 渐变起点
	static final C_BTN_GRAD_B:FlxColor = 0xFF8B5CF6; // Primary 渐变终点
	// —— 危险 / 状态反馈 ——
	static final C_DANGER:FlxColor = 0xFFEF4444;
	static final C_OK:FlxColor = 0xFF22C55E;
	static final C_WARN:FlxColor = 0xFFF59E0B;
	static final C_INFO:FlxColor = 0xFF3B82F6;
	// —— 兼容别名（控件强调/勾选/未选中控件底/测试标识） ——
	static final C_ACCENT:FlxColor = 0xFF8B5CF6;     // 控件强调（violet）
	static final C_ACCENT_CHECK:FlxColor = 0xFF4F46E5; // 勾选方块选中（indigo）
	static final C_BG_2:FlxColor = 0xFF2E3340;       // 未选中控件底色
	static final C_TEST_ACCENT:FlxColor = 0xFFFFD166; // 测试菜单标识（黄）
	...
	// ============ 菜单定义 ============
	public var menus:Array<StageMenuDef> = [];
	...
	// 一级主菜单按钮背景：hover 底色 / 选中渐变底 / 选中左侧装饰条（与按钮一一对应）
	var menuHoverBgs:Array<FlxSprite> = [];
	var menuSelBgs:Array<FlxSprite> = [];
	var menuSelBars:Array<FlxSprite> = [];

	// ============ 下拉浮窗 ============
	var dropGroup:FlxSpriteGroup;
	var dropBg:FlxSprite;
	var dropBorder:FlxSprite;
	var dropRows:Array<StageDropdownRow> = [];
	...
	var customControls:Array<StageCustomControl> = [];
	...
	// ============ 状态栏（独立组件） ============
	public var statusBar:StageEditorStatusBar;
	...
	var stepperOverlayCtrl:StageCustomControl = null;

	// ============ 自绘 dropdown 面板：鼠标拖拽滚动状态 ============
	// 按下记录起点 → 移动超过阈值进入拖拽（实时滚动）；原地释放 = 点击（选中/收起）
	var dragDropdownCtrl:StageCustomControl = null;
	var dragStartY:Float = 0;
	var dragStartScrollIdx:Int = 0;
	...
	/** 菜单打开后回调，用于让外部（StageEditorState）刷新 widget 的值 */
	public var onMenuOpened:String->Void = null;

	/** 当前用于鼠标判定的摄像机，默认 FlxG.camera；StageEditorState 可在 create 末尾覆盖。 */
	public var uiCamera:flixel.FlxCamera;

	/** 顶层覆盖层容器（StageEditorState 赋值）：stepper 覆盖层挂这里，渲染在菜单之上 */
	...
	public function new()
	{
		...
		statusBar = new StageEditorStatusBar();
	...
	{
		barBg = new FlxSprite().makeGraphic(FlxG.width, BAR_HEIGHT, C_CARD);
		barBg.x = 0;
		barBg.y = 0;
		add(barBg);
	...
	function buildDropdown():Void
	{
		...
		dropBorder = new FlxSprite().makeGraphic(1, 1, C_BORDER);
		dropGroup.add(dropBorder);
		dropBg = new FlxSprite().makeGraphic(1, 1, C_CARD);
	...
	{
		descBg = new FlxSprite().makeGraphic(1, 1, C_CARD);
		descBg.visible = false;
		add(descBg);
		descIcon = new FlxText(0, 0, 0, '[i]', 12);
		descIcon.setFormat(Paths.font(langFont()), 12, C_ACCENT, LEFT);
	...
	// ============ 菜单数据定义 ============
	// 动态菜单：菜单打开时若 dynamicMenuKey 匹配，items 由 dynamicItems() 现场生成
	// （用于对象列表等需要随数据增删变化的动态行）
	public var dynamicMenuKey:String = '';
	public var dynamicItems:Void->Array<StageMenuItemDef>;
	public function buildMenus():Void
	{
		menus = [
			{key: 'stage', isTest: false, items: [
				item_label_widget('w_stage_select', 'label_stage_select', 'desc_stage_select'),
				SEP,
				item_label_widget('w_directory', 'label_directory', 'desc_directory'),
				item_widget('w_hide_gf', 'desc_hide_gf'),
				item_label_widget('w_default_zoom', 'label_default_zoom', 'desc_default_zoom'),
				item_label_widget('w_cam_speed', 'label_cam_speed', 'desc_cam_speed'),
			]},
			{key: 'object', isTest: false, items: [
				item_cmd('obj_new_sprite', 'desc_obj_new_sprite', () -> callAction('obj_new_sprite')),
				item_cmd('obj_new_animated', 'desc_obj_new_animated', () -> callAction('obj_new_animated')),
				item_cmd('obj_new_square', 'desc_obj_new_square', () -> callAction('obj_new_square')),
				item_cmd('obj_delete', 'desc_obj_delete', () -> callAction('obj_delete')),
				item_cmd('obj_duplicate', 'desc_obj_duplicate', () -> callAction('obj_duplicate')),
				item_cmd('obj_anim', 'desc_obj_anim', () -> callAction('obj_anim')),
				item_cmd('obj_move_up', 'desc_obj_move_up', () -> callAction('obj_move_up')),
				item_cmd('obj_move_down', 'desc_obj_move_down', () -> callAction('obj_move_down')),
				SEP,
				item_dyn_text('obj_name', 'desc_obj_name'),
				item_dyn_text('obj_type', 'desc_obj_type'),
				item_dyn_text('obj_image', 'desc_obj_image'),
				item_dyn_text('obj_pos', 'desc_obj_pos'),
				item_dyn_text('obj_scale', 'desc_obj_scale'),
				item_dyn_text('obj_alpha', 'desc_obj_alpha'),
				item_dyn_text('obj_angle', 'desc_obj_angle'),
				SEP,
				item_widget('w_show_bf', 'desc_show_bf'),
				item_widget('w_show_dad', 'desc_show_dad'),
				item_widget('w_show_gf', 'desc_show_gf'),
			]},
			{key: 'camera', isTest: false, items: [
				item_label_widget('w_cam_zoom', 'label_cam_zoom', 'desc_cam_zoom'),
				item_label_widget('w_cam_target', 'label_cam_target', 'desc_cam_target'),
				item_label_widget('w_cam_bf_x', 'label_cam_bf_x', 'desc_cam_offset'),
				item_label_widget('w_cam_bf_y', 'label_cam_bf_y', 'desc_cam_offset'),
				item_label_widget('w_cam_dad_x', 'label_cam_dad_x', 'desc_cam_offset'),
				item_label_widget('w_cam_dad_y', 'label_cam_dad_y', 'desc_cam_offset'),
				item_label_widget('w_cam_gf_x', 'label_cam_gf_x', 'desc_cam_offset'),
				item_label_widget('w_cam_gf_y', 'label_cam_gf_y', 'desc_cam_offset'),
			]},
			{key: 'view', isTest: false, items: [
				item_widget('w_low_quality', 'desc_low_quality'),
				item_widget('w_high_quality', 'desc_high_quality'),
				SEP,
				item_cmd('focus_bf', 'desc_focus_bf', () -> callAction('focus_bf')),
				item_cmd('focus_dad', 'desc_focus_dad', () -> callAction('focus_dad')),
				item_cmd('focus_gf', 'desc_focus_gf', () -> callAction('focus_gf')),
			]},
			{key: 'file', isTest: false, items: [
				item_cmd('save', 'desc_save', () -> callAction('save')),
				item_cmd('reload_stage', 'desc_reload_stage', () -> callAction('reload_stage')),
				item_cmd('load_template', 'desc_load_template', () -> callAction('load_template')),
			]},
			{key: 'help', isTest: false, items: [
				item_cmd('tutorial', 'desc_tutorial', () -> callAction('tutorial')),
				SEP,
				...
				SEP,
				item_text('help_obj', 'desc_help_obj'),
				item_text('help_obj_move', 'desc_help_obj_move'),
				item_text('help_obj_sel', 'desc_help_obj_sel'),
				item_text('help_obj_scale', 'desc_help_obj_scale'),
				SEP,
	...
	// ============ 行类型辅助函数 ============
	static var SEP:StageMenuItemDef = {type: Sep, labelKey: null, descKey: null, getChecked: null, onCheck: null, onClick: null};

	function item_cmd(labelKey:String, descKey:String, onClick:Void->Void):StageMenuItemDef
	{
		return {type: Cmd, labelKey: labelKey, descKey: descKey, getChecked: null, onCheck: null, onClick: onClick};
	}

	function item_raw_cmd(rawText:String, descKey:String, onClick:Void->Void, ?getChecked:Void->Bool):StageMenuItemDef
	{
		return {type: RawCmd, labelKey: null, descKey: descKey, rawText: rawText, getChecked: getChecked, onCheck: null, onClick: onClick};
	}

	function item_widget(widgetKey:String, descKey:String):StageMenuItemDef
	{
		return {type: Widget, labelKey: widgetKey, descKey: descKey, getChecked: null, onCheck: null, onClick: null, widgetKey: widgetKey};
	}

	function item_label_widget(widgetKey:String, labelKey:String, descKey:String):StageMenuItemDef
	{
		return {type: LabelWidget, labelKey: labelKey, descKey: descKey, getChecked: null, onCheck: null, onClick: null, widgetKey: widgetKey};
	}

	function item_text(textKey:String, descKey:String):StageMenuItemDef
	{
		return {type: TextLine, labelKey: textKey, descKey: descKey, getChecked: null, onCheck: null, onClick: null};
	}

	function item_dyn_text(dynKey:String, descKey:String):StageMenuItemDef
	{
	...
	function translateItemKey(key:String):String
	{
		...
		var translated = Language.get(itemKey, 'stage');
		if (translated == itemKey || translated.indexOf('(404)') != -1) {
			// 尝试不带 item_ 前缀
			translated = Language.get(key.substring(2), 'stage');
		}
		return translated;
	}
	...
	{
		var translated = Language.get(key, 'stage');
		if (translated == key || translated.indexOf('(404)') != -1) {
			// 尝试带 label_ 前缀
			translated = Language.get('label_' + key, 'stage');
		}
	...
	public function applyLang():Void
	{
		...
		while (menuHoverBgs.length > 0)
		{
			var b = menuHoverBgs.pop();
			remove(b, true);
			b.destroy();
		}
		while (menuSelBgs.length > 0)
		{
			var b = menuSelBgs.pop();
			remove(b, true);
			b.destroy();
		}
		while (menuSelBars.length > 0)
		{
			var b = menuSelBars.pop();
			remove(b, true);
			b.destroy();
		}
		...
		{
			var m = menus[i];
			var bx:Float = i * MENU_W;

			// hover 底色（未选中时悬停显示，通用 hover 半透明）
			var hovBg:FlxSprite = new FlxSprite().makeGraphic(MENU_W, BAR_HEIGHT, C_HOVER_GEN);
			hovBg.x = bx;
			hovBg.y = 0;
			hovBg.visible = false;
			add(hovBg);
			menuHoverBgs.push(hovBg);

			// 选中渐变背景（indigo → violet，左→右）
			var selBg:FlxSprite = FlxGradient.createGradientFlxSprite(MENU_W, BAR_HEIGHT, [C_MENU_GRAD_A, C_MENU_GRAD_B], 1, 180);
			selBg.x = bx;
			selBg.y = 0;
			selBg.visible = false;
			add(selBg);
			menuSelBgs.push(selBg);

			// 选中左侧装饰条（浅紫）
			var selBar:FlxSprite = new FlxSprite().makeGraphic(3, BAR_HEIGHT, C_MENU_BAR);
			selBar.x = bx;
			selBar.y = 0;
			selBar.visible = false;
			add(selBar);
			menuSelBars.push(selBar);

			var txt:FlxText = new FlxText(bx, 0, MENU_W, Language.get('menu_' + m.key, 'stage'), 14);
			txt.setFormat(Paths.font(langFont()), 14, m.isTest ? C_TEST_ACCENT : C_TEXT_SUB, CENTER);
	 ...
	 * 控件生命周期由 StageEditorState 管理，菜单只负责 add/remove 到 dropGroup。
	 */
	...
	public function refreshDynTexts():Void
	{
		for (key in dynTexts.keys())
		{
			...
			if (txt == null) txt = '';
			var full:String = translateLabelKey(key) + ' ' + txt;
			if (t.text != full)
				t.text = full;
		}
	}
	...
	override function update(elapsed:Float):Void
	{
			...
			else if (activeMenu >= 0)
			{
				if (insideUI)
				{
					...
					else
					{
						...
						// ② 展开的 dropdown 面板内按下 → 先进入"潜在拖拽"：
						//    移动超过阈值=滚动列表；原地释放=点击（选中/收起），释放时才判定
						var pressedOnDropdownPanel:Bool = false;
						for (ctrl in customControls)
						{
							...
							{
								dragDropdownCtrl = ctrl;
								dragStartY = my;
								dragStartScrollIdx = (ctrl.scrollIdx != null) ? ctrl.scrollIdx : 0;
								dragActive = false;
								pressedOnDropdownPanel = true;
								break;
							}
						}
						var panelHandled:Bool = pressedOnDropdownPanel;
						if (!pressedOnDropdownPanel)
							panelHandled = handleDropdownPanelClick(mx, my);
						if (!panelHandled)
						{
							// ③ 普通行点击
							var hitRow:StageDropdownRow = null;
							for (row in dropRows)
							{
								if (row.hit != null)
								{
							...
							if (hitRow != null)
							{
								...
								if (hitRow.def.type == LabelWidget)
									handled = handleStageCustomControlClick(hitRow);
								if (!handled)
								{
									activateRow(hitRow);
									// Cmd 点完关菜单；Widget/LabelWidget/RawCmd（对象列表）保持打开
		...
		// 更新顶栏按钮 hover / active 视觉（一级主菜单：选中=渐变底+白字+左侧装饰条）
		for (i in 0...menuTitleSprites.length)
		{
			var t = menuTitleSprites[i];
			var isActive = (i == activeMenu);
			var isHover = (i == hoverIdx);
			if (menuSelBgs != null && i < menuSelBgs.length)
			{
				menuSelBgs[i].visible = isActive;
				menuSelBars[i].visible = isActive;
				menuHoverBgs[i].visible = !isActive && isHover;
			}
			var baseColor = menus[i].isTest ? C_TEST_ACCENT : C_TEXT_SUB;
			t.color = isActive ? FlxColor.WHITE : (isHover ? C_TEXT_MAIN : baseColor);
		}

		// 下拉行 hover → 更新描述条 + 绘制/清除 hover 背景
		if (activeMenu >= 0 && dropGroup.visible)
		{
			var hoverRow:StageDropdownRow = null;
			for (row in dropRows)
			{
			...
			// 更新每行 hover 背景的可见性（RawCmd 选中行：非 hover 时显示选中底）
			for (row in dropRows)
			{
				var selBase:Bool = false;
				if (row.def != null && row.def.type == RawCmd && row.def.getChecked != null)
				{
					try { selBase = row.def.getChecked(); } catch (e:Dynamic) {}
				}
				if (row.hoverBg != null)
					row.hoverBg.visible = (row == hoverRow);
				if (row.selBg != null)
					row.selBg.visible = selBase && row != hoverRow;
			}
			...
			if (FlxG.mouse.wheel != 0)
			{
				for (ctrl in customControls)
				{
					...
					renderStageDropdownOptions(ctrl);
					break;
				}
			}

			// ★ 自绘 dropdown 展开面板：鼠标拖拽滚动（按住面板内容上下拖动）
			if (dragDropdownCtrl != null)
			{
				if (dragDropdownCtrl.open != true)
				{
					// 面板已被收起（如点了别处）→ 结束拖拽状态
					dragDropdownCtrl = null;
					dragActive = false;
				}
				else
				{
					// 移动超过阈值才视为拖拽（区分"原地点击"）
					if (!dragActive && FlxG.mouse.pressed && Math.abs(mViewY - dragStartY) > 4)
						dragActive = true;

					if (dragActive && FlxG.mouse.pressed)
					{
						var dTotal:Int = (dragDropdownCtrl.options != null) ? dragDropdownCtrl.options.length : 0;
						var dMaxScroll:Int = Std.int(Math.max(0, dTotal - MAX_DROP_OPT));
						var dDelta:Int = Std.int((dragStartY - mViewY) / DROP_OPT_H);
						var dNewIdx:Int = Std.int(FlxMath.bound(dragStartScrollIdx + dDelta, 0, dMaxScroll));
						if (dNewIdx != dragDropdownCtrl.scrollIdx)
						{
							dragDropdownCtrl.scrollIdx = dNewIdx;
							clearDropdownPanel(dragDropdownCtrl);
							renderStageDropdownOptions(dragDropdownCtrl);
						}
					}
					...
					if (FlxG.mouse.justReleased)
					{
						if (!dragActive)
						{
							// 原地释放 = 点击：选中选项 / 点空白收起（复用原点击判定）
							handleDropdownPanelClick(mViewX, mViewY);
						}
						dragDropdownCtrl = null;
						dragActive = false;
					}
	...
	// ============ 自绘控件：显示刷新（从原生控件读数据） ============
	function refreshStageCustomControlDisplays():Void
	{
		for (ctrl in customControls)
		{
	...
	function updateStepperDisplay(ctrl:StageCustomControl):Void
	{
		var txt:String = '';
	...
	function setControlText(ctrl:StageCustomControl, txt:String):Void
	{
	...
	function updateDropdownDisplay(ctrl:StageCustomControl):Void
	{
		var lbl:String = '';
	...
	function updateInputDisplay(ctrl:StageCustomControl):Void
	{
		var txt:String = '';
	...
	function readSliderValue(ctrl:StageCustomControl):Float
	{
		var v:Float = 0;
	...
	function updateSliderDisplay(ctrl:StageCustomControl):Void
	{
	...
	function handleStageCustomControlClick(row:StageDropdownRow):Bool
	{
		var ctrl:StageCustomControl = null;
		...
		if (ctrl.ctype == 'dropdown')
		{
			if (ctrl.box != null && pointInSprite(mx, my, ctrl.box))
			{
				...
				else { closeCustomDropdowns(); ctrl.open = true; renderStageDropdownOptions(ctrl); }
				return true;
			}
			return false;
		}
	...
	function openStepperOverlay(ctrl:StageCustomControl):Void
	{
	...
	function commitStepperOverlay():Void
	{
		...
		var ctrl:StageCustomControl = stepperOverlayCtrl;
		...
		if (w != null && stepperOverlay != null)
		{
			...
			if (!Math.isNaN(parsed))
			{
				...
				// 广播 CHANGE_EVENT 让 StageEditorState.getEvent 更新数据
	...
	function sliderDrag(ctrl:StageCustomControl, mx:Float):Void
	{
		if (ctrl.track == null) return;
	...
	function handleDropdownPanelClick(mx:Float, my:Float):Bool
	{
		...
		for (c in customControls)
		{
			if (c.ctype == 'dropdown' && c.open == true) { anyOpen = true; break; }
		}
		...
		for (ctrl in customControls)
		{
			...
			if (ctrl.panelRows != null)
			{
				for (pr in ctrl.panelRows)
				{
					...
					{
						selectStageDropdownOption(ctrl, pr.idx);
						return true;
					}
				}
			}
	...
	function selectStageDropdownOption(ctrl:StageCustomControl, optIdx:Int):Void
	{
	...
	function renderStageDropdownOptions(ctrl:StageCustomControl):Void
	{
		...
		var pBorder:FlxSprite = new FlxSprite().makeGraphic(Std.int(pw) + 2, visCount * DROP_OPT_H + 2, C_BORDER);
		...
		var pBg:FlxSprite = new FlxSprite().makeGraphic(Std.int(pw), visCount * DROP_OPT_H, C_CARD);
		pBg.x = Std.int(px);
		pBg.y = Std.int(py);
		...
		var rows:Array<StageDropdownOption> = [];
		for (i in 0...visCount)
		{
			...
			var bg:FlxSprite = new FlxSprite().makeGraphic(Std.int(pw), DROP_OPT_H, C_HOVER_GEN);
			bg.x = Std.int(px);
			bg.y = Std.int(oy);
			bg.visible = false;
			...
			t.setFormat(Paths.font(langFont()), 11, C_TEXT_SUB, LEFT);
			fitTextOneLine(t);
			t.x = Std.int(px + 5);
	...
	function clearDropdownPanel(ctrl:StageCustomControl):Void
	{
		if (ctrl.panelRows != null)
		{
			for (pr in ctrl.panelRows)
	...
	// ============ 自绘控件：数值 / 输入覆盖层 ============
	function stepperPlus(ctrl:StageCustomControl):Void
	{
		stepperStep(ctrl, 1);
	}

	function stepperMinus(ctrl:StageCustomControl):Void
	{
		stepperStep(ctrl, -1);
	}

	function stepperStep(ctrl:StageCustomControl, dir:Int):Void
	{
		var w:Dynamic = ctrl.widget;
		if (w == null) return;
		try
		{
			...
			// 原生 set_value 只改文本不广播事件，手动补一发 CHANGE_EVENT 让 StageEditorState 更新数据
			try { FlxUI.event(FlxUINumericStepper.CHANGE_EVENT, w, w.value, w.params); } catch (e:Dynamic) {}
		}
		catch (e:Dynamic) {}
		updateStepperDisplay(ctrl);
	...
	function openInputOverlay(ctrl:StageCustomControl):Void
	{
		var w:Dynamic = ctrl.widget;
		clearActiveEditWidget();
		if (stepperOverlayCtrl != null)
	 ...
	 * StageEditorState.update 开头用这个判断点击是否在"列表内"，不在就强制 closeMenu。
	 */
	...
	// ============ 打开 / 关闭菜单 ============
	/** 是否有输入覆盖层正在编辑（输入框/stepper 覆盖层），此时外部快捷键应让位。 */
	public function isInputOverlayActive():Bool
	{
		return activeEditWidget != null || stepperOverlayCtrl != null;
	}
	...
	public function openMenu(idx:Int):Void
	{
		...
		refreshStageCustomControlDisplays();
	}
	...
	function rebuildDropdown():Void
	{
		...
		// 动态菜单：每次打开现场生成 items，并追加到静态 items 之后
		// （对象列表随对象增删变化，但不能替换掉新建/删除/复制等静态项）
		var items = m.items;
		if (m.key == dynamicMenuKey && dynamicItems != null)
		{
			try
			{
				var dyn = dynamicItems();
				if (dyn != null && dyn.length > 0)
					items = items.concat(dyn);
			}
			catch (e:Dynamic) {}
		}
		...
		for (it in items)
		{
			...
			if (it.type == Widget || it.type == LabelWidget)
			{
				...
				{
					// 标签最小宽度
					var lbl = Language.get(it.labelKey, 'stage');
			...
			{
				var txt = Language.get(it.labelKey, 'stage');
				var lineH = 16;
				// 估算文本行数
				var charW = 7;
			...
			{
				var lbl = (it.type == RawCmd && it.rawText != null) ? it.rawText : Language.get('item_' + it.labelKey, 'stage');
		...
		// 位置：紧跟菜单栏正下方（下拉面板覆盖状态栏，无间隙）
		var dropX = Std.int(activeMenu * MENU_W);
		var dropY = Std.int(BAR_HEIGHT + 2);
		...
		for (it in items)
		{
			if (it.type == Sep)
			{
				var sepLine = new FlxSprite(Std.int(dropX + 6), Std.int(curY + 3)).makeGraphic(Std.int(dropW - 12), 2, C_BORDER);
			...
			if (it.type == Widget || it.type == LabelWidget)
			{
				...
				// ===== hover 背景（先加） =====
				var hoverBg:FlxSprite = new FlxSprite().makeGraphic(Std.int(dropW - 4), rowH, C_HOVER_GEN);
				...
				if (it.type == Widget)
				{
					...
					lbl.setFormat(Paths.font(langFont()), 12, C_TEXT_SUB, LEFT);
					lbl.x = Std.int(dropX + ROW_PAD_X);
					lbl.y = Std.int(curY + (rowH - lbl.height) / 2);
					addToDrop(lbl, false);

					// 装饰性勾选方块（右）—— toggle 时通过 row.checkIcon 更新颜色
					var checkIcon:FlxSprite = new FlxSprite().makeGraphic(14, 14, C_BG_2);
					checkIcon.antialiasing = ClientPrefs.data.antialiasing;
					checkIcon.color = isChecked ? C_ACCENT_CHECK : C_BG_2;
				...
				else // LabelWidget
				{
					...
					if (widget != null)
					{
						...
						// 底板 + 1px 边框
						var border:FlxSprite = new FlxSprite().makeGraphic(ctrlW, ctrlH, C_BORDER);
						border.x = ctrlX;
						border.y = ctrlY;
						border.antialiasing = ClientPrefs.data.antialiasing;
						addToDrop(border, false);

						var box:FlxSprite = new FlxSprite().makeGraphic(ctrlW - 2, ctrlH - 2, C_BG_2);
						box.x = ctrlX + 1;
						box.y = ctrlY + 1;
						...
						txt.setFormat(Paths.font(langFont()), 12, C_TEXT_SUB, LEFT);
						...
						if (Std.is(widget, FlxUINumericStepper))
						{
							...
							var btnPlus:FlxSprite = new FlxSprite().makeGraphic(btnW, btnH, C_ACCENT);
							btnPlus.x = btnPlusX;
							btnPlus.y = ctrlY + 1;
							...
							var btnMinus:FlxSprite = new FlxSprite().makeGraphic(btnW, btnH, C_ACCENT);
						...
						else if (Std.is(widget, FlxUISlider))
						{
							...
							var track:FlxSprite = new FlxSprite().makeGraphic(trackW, trackH, C_BORDER);
							track.x = trackX;
							track.y = trackY;
							...
							var fill:FlxSprite = new FlxSprite().makeGraphic(1, trackH, C_ACCENT);
							fill.origin.set(0, 0);
							fill.x = trackX;
							fill.y = trackY;
							...
							var thumb:FlxSprite = new FlxSprite().makeGraphic(8, ctrlH - 2, C_TEXT_MAIN);
							thumb.x = trackX;
							thumb.y = ctrlY + 1;
			...
			if (it.type == DynText)
			{
				// 动态文本行：显示 "标签: 值"（单行，紧凑），内容由外部 provider 提供
				var txtLabel = new FlxText(0, 0, Std.int(dropW - ROW_PAD_X * 2), ' ', 11);
				txtLabel.setFormat(Paths.font(langFont()), 11, C_TEXT_SUB, LEFT);
				txtLabel.alignment = LEFT;
				...
				if (dynTextProviders.exists(it.labelKey) && dynTextProviders.get(it.labelKey) != null)
				{
					try
					{
						...
						if (firstText != null) txtLabel.text = translateLabelKey(it.labelKey) + ' ' + firstText;
					}
					catch (e:Dynamic) {}
				}
			...
			// === RawCmd 行（动态列表，如对象列表）：不翻译 + 左侧选中标记 ===
			if (it.type == RawCmd)
			{
				var labelStr = (it.rawText != null) ? it.rawText : '';
				var isSelected = (it.getChecked != null) ? it.getChecked() : false;

				// hover 背景（先加）
				var hoverBg:FlxSprite = new FlxSprite().makeGraphic(Std.int(dropW) - 4, ROW_H, C_SUB_HOVER);
				hoverBg.antialiasing = ClientPrefs.data.antialiasing;
				hoverBg.x = Std.int(dropX + 2);
				hoverBg.y = Std.int(curY);
				hoverBg.visible = false;
				...
				// 选中底色（二级子菜单选中态：0x268B5CF6，hover 时让位给 hoverBg）
				var selBg:FlxSprite = new FlxSprite().makeGraphic(Std.int(dropW) - 4, ROW_H, C_SUB_SEL_BG);
				selBg.antialiasing = ClientPrefs.data.antialiasing;
				selBg.x = Std.int(dropX + 2);
				selBg.y = Std.int(curY);
				selBg.visible = isSelected;
				addToDrop(selBg, false);

				// 左侧选中标记 ✓
				var selIcon = new FlxText(0, 0, 14, isSelected ? '✓' : '', 12);
				selIcon.setFormat(Paths.font(langFont()), 12, C_SUB_SEL_TEXT, CENTER);
				selIcon.x = Std.int(dropX + ROW_PAD_X - 2);
				selIcon.y = Std.int(curY + (ROW_H - selIcon.height) / 2 - 1);
				addToDrop(selIcon, false);

				// 标签（选中行：浅紫文字）
				var lbl = new FlxText(0, 0, Std.int(dropW - ROW_PAD_X * 2 - 30), labelStr, 12);
				lbl.setFormat(Paths.font(langFont()), 12, isSelected ? C_SUB_SEL_TEXT : C_TEXT_SUB, LEFT);
				lbl.x = Std.int(dropX + ROW_PAD_X + 16);
			...
			// === Cmd 行 ===
			var labelStr = Language.get('item_' + it.labelKey, 'stage');
			...
			lbl.setFormat(Paths.font(langFont()), 12, isDanger ? C_DANGER : C_TEXT_SUB, LEFT);
			lbl.x = Std.int(dropX + ROW_PAD_X);
			lbl.y = Std.int(curY + 3);
			addToDrop(lbl, false);
		...
		dropBg.makeGraphic(Std.int(dropW), finalH, C_CARD);
		dropBg.antialiasing = ClientPrefs.data.antialiasing;
		dropBg.x = dropX;
		dropBg.y = dropY;
		dropBorder.makeGraphic(Std.int(dropW) + 2, finalH + 2, C_BORDER);
		dropBorder.antialiasing = ClientPrefs.data.antialiasing;
		dropBorder.x = dropX - 1;
		dropBorder.y = dropY - 1;
	}

	function refreshCheckIcon(icon:FlxText, it:StageMenuItemDef):Void
	{
	...
	function clearStageDropdownRows():Void
	{
	...
	function activateRow(row:StageDropdownRow):Void
	{
		...
		switch (it.type)
		{
			...
			case RawCmd:
				// 动态列表行：执行点击后刷新选中标记（✓ 与高亮）
				if (it.onClick != null)
				{
					it.onClick();
					if (it.getChecked != null)
					{
						var sel:Bool = it.getChecked();
						if (row.checkIcon != null)
						{
							var icon:FlxText = cast row.checkIcon;
							icon.text = sel ? '✓' : '';
						}
						if (row.label != null)
							row.label.color = sel ? C_SUB_SEL_TEXT : C_TEXT_SUB;
						if (row.selBg != null)
							row.selBg.visible = sel;
					}
				}
			case Check:
				if (it.onCheck != null)
			...
			case Widget:
				...
				if (row.widget != null)
				{
					...
					if (row.checkIcon != null)
					{
						...
						row.checkIcon.color = isChecked ? C_ACCENT_CHECK : C_BG_2;
					}
			...
			case LabelWidget:
				// 交互已由 handleStageCustomControlClick（自绘控件）接管：
	...
	// ============ 描述条 ============
	function updateDescBar(row:StageDropdownRow):Void
	{
		...
		var txt = Language.get(row.def.descKey, 'stage');
		...
		descBg.makeGraphic(descW, descH, C_CARD);
		descBg.x = dropBg.x;
...
enum StageItemType
{
	Sep;
	Cmd;
	RawCmd; // 直接文本的命令行（不翻译，用于动态列表，如对象列表）；getChecked 驱动左侧选中标记
...
typedef StageMenuDef =
{
	key:String,
	isTest:Bool,
	items:Array<StageMenuItemDef>
};

typedef StageDropdownRow =
{
	type:StageItemType,
	labelKey:String,
	descKey:String,
	def:StageMenuItemDef,
	hit:FlxSprite,
...
typedef StageCustomControl =
{
	...
	?panelBg:FlxSprite,
	?panelRows:Array<StageDropdownOption>,
	?panelH:Int,
	// slider
	?track:FlxSprite,
	?fill:FlxSprite,
