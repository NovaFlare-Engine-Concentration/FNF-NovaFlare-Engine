 ...
 */
class DialogueEditorMenuBar extends FlxSpriteGroup
{
	...
	// ============ 菜单定义 ============
	public var menus:Array<DialogueMenuDef> = [];
	...
	var dropRows:Array<DialogueDropdownRow> = [];
	...
	var customControls:Array<DialogueCustomControl> = [];
	...
	// ============ 状态栏（独立组件） ============
	public var statusBar:DialogueEditorStatusBar;
	...
	var stepperOverlayCtrl:DialogueCustomControl = null;
	...
	var panelPressCtrl:DialogueCustomControl = null; // 按下时点中的展开面板
	...
	public function new()
	{
		...
		statusBar = new DialogueEditorStatusBar();
	...
	{
		menus = [
			{key: 'line', isTest: false, items: [
				item_cmd('prev_line', 'desc_prev_line', () -> callAction('prev_line')),
				item_cmd('next_line', 'desc_next_line', () -> callAction('next_line')),
				SEP,
				item_cmd('add_line', 'desc_add_line', () -> callAction('add_line')),
				item_cmd('remove_line', 'desc_remove_line', () -> callAction('remove_line')),
			]},
			{key: 'char', isTest: false, items: [
				item_label_widget('w_char', 'char', 'desc_char'),
				item_widget('w_angry', 'desc_angry'),
				item_label_widget('w_speed', 'speed', 'desc_speed'),
				item_label_widget('w_sound', 'sound', 'desc_sound'),
				SEP,
				item_cmd('replay_text', 'desc_replay_text', () -> callAction('replay_text')),
				item_cmd('prev_anim', 'desc_prev_anim', () -> callAction('prev_anim')),
				item_cmd('next_anim', 'desc_next_anim', () -> callAction('next_anim')),
			]},
			{key: 'text', isTest: false, items: [
				item_label_widget('w_text', 'text', 'desc_text'),
			]},
			...
			{key: 'help', isTest: false, items: [
				item_text('help_nav', 'desc_help_nav'),
				item_text('help_anims', 'desc_help_anims'),
				item_text('help_replay', 'desc_help_replay'),
	...
	// ============ 行类型辅助函数 ============
	static var SEP:DialogueMenuItemDef = {type: Sep, labelKey: null, descKey: null, getChecked: null, onCheck: null, onClick: null};

	function item_cmd(labelKey:String, descKey:String, onClick:Void->Void):DialogueMenuItemDef
	{
		return {type: Cmd, labelKey: labelKey, descKey: descKey, getChecked: null, onCheck: null, onClick: onClick};
	}

	function item_widget(widgetKey:String, descKey:String):DialogueMenuItemDef
	{
		return {type: Widget, labelKey: widgetKey, descKey: descKey, getChecked: null, onCheck: null, onClick: null, widgetKey: widgetKey};
	}

	function item_label_widget(widgetKey:String, labelKey:String, descKey:String):DialogueMenuItemDef
	{
		return {type: LabelWidget, labelKey: labelKey, descKey: descKey, getChecked: null, onCheck: null, onClick: null, widgetKey: widgetKey};
	}

	function item_text(textKey:String, descKey:String):DialogueMenuItemDef
	{
		return {type: TextLine, labelKey: textKey, descKey: descKey, getChecked: null, onCheck: null, onClick: null};
	}

	function item_dyn_text(dynKey:String, descKey:String):DialogueMenuItemDef
	{
	...
	function translateItemKey(key:String):String
	{
		...
		var translated = Language.get(itemKey, 'dialogue');
		if (translated == itemKey || translated.indexOf('(404)') != -1) {
			// 尝试不带 item_ 前缀
			translated = Language.get(key.substring(2), 'dialogue');
		}
		return translated;
	}
	...
	{
		var translated = Language.get(key, 'dialogue');
		if (translated == key || translated.indexOf('(404)') != -1) {
			// 尝试带 label_ 前缀
			translated = Language.get('label_' + key, 'dialogue');
		}
	...
	public function applyLang():Void
	{
		...
		for (i in 0...menus.length)
		{
			...
			var m = menus[i];
			var txt:FlxText = new FlxText(i * MENU_W, 0, MENU_W, Language.get('menu_' + m.key, 'dialogue'), 14);
	...
	override function update(elapsed:Float):Void
	{
			...
			else if (activeMenu >= 0)
			{
				if (insideUI)
				{
					...
					else
					{
						...
						{
							// ③ 普通行点击
							var hitRow:DialogueDropdownRow = null;
							for (row in dropRows)
							{
								if (row.hit != null)
								{
							...
							if (hitRow != null)
							{
								...
								if (hitRow.def.type == LabelWidget)
									handled = handleDialogueCustomControlClick(hitRow);
								if (!handled)
								{
									activateRow(hitRow);
		...
		{
			var hoverRow:DialogueDropdownRow = null;
			for (row in dropRows)
			{
			...
			if (FlxG.mouse.wheel != 0)
			{
				for (ctrl in customControls)
				{
					...
					renderDialogueDropdownOptions(ctrl);
					break;
				}
			}
		}
		...
		if (panelPressCtrl != null && FlxG.mouse.pressed)
		{
			...
			if (panelDragMoved)
			{
				var ctrl:DialogueCustomControl = panelPressCtrl;
				var total:Int = (ctrl.options != null) ? ctrl.options.length : 0;
				var maxScroll:Int = Std.int(Math.max(0, total - MAX_DROP_OPT));
	...
	// ============ 自绘控件：显示刷新（从原生控件读数据） ============
	function refreshDialogueCustomControlDisplays():Void
	{
		for (ctrl in customControls)
		{
	...
	function updateStepperDisplay(ctrl:DialogueCustomControl):Void
	{
		var txt:String = '';
	...
	function setControlText(ctrl:DialogueCustomControl, txt:String):Void
	{
	...
	function updateDropdownDisplay(ctrl:DialogueCustomControl):Void
	{
		var lbl:String = '';
	...
	function updateInputDisplay(ctrl:DialogueCustomControl):Void
	{
		var txt:String = '';
	...
	function readSliderValue(ctrl:DialogueCustomControl):Float
	{
		var v:Float = 0;
	...
	function updateSliderDisplay(ctrl:DialogueCustomControl):Void
	{
	...
	function handleDialogueCustomControlClick(row:DialogueDropdownRow):Bool
	{
		var ctrl:DialogueCustomControl = null;
		...
		if (ctrl.ctype == 'dropdown')
		{
			if (ctrl.box != null && pointInSprite(mx, my, ctrl.box))
			{
				...
				else { closeCustomDropdowns(); ctrl.open = true; renderDialogueDropdownOptions(ctrl); }
				return true;
			}
			return false;
		}
	...
	function openStepperOverlay(ctrl:DialogueCustomControl):Void
	{
	...
	function commitStepperOverlay():Void
	{
		...
		var ctrl:DialogueCustomControl = stepperOverlayCtrl;
	...
	function sliderDrag(ctrl:DialogueCustomControl, mx:Float):Void
	{
		if (ctrl.track == null) return;
	...
	function releaseDropdownPanelClick():Void
	{
		...
		if (ctrl.panelRows != null)
		{
			for (pr in ctrl.panelRows)
			{
				...
				{
					selectDialogueDropdownOption(ctrl, pr.idx);
					return;
				}
			}
		}
	...
	function selectDialogueDropdownOption(ctrl:DialogueCustomControl, optIdx:Int):Void
	{
	...
	function renderDialogueDropdownOptions(ctrl:DialogueCustomControl):Void
	{
		...
		var rows:Array<DialogueDropdownOption> = [];
	...
	function clearDropdownPanel(ctrl:DialogueCustomControl):Void
	{
		if (ctrl.panelRows != null)
		{
			for (pr in ctrl.panelRows)
	...
	// ============ 自绘控件：数值 / 输入覆盖层 ============
	function stepperPlus(ctrl:DialogueCustomControl):Void
	{
		stepperStep(ctrl, 1);
	}

	function stepperMinus(ctrl:DialogueCustomControl):Void
	{
		stepperStep(ctrl, -1);
	}

	function stepperStep(ctrl:DialogueCustomControl, dir:Int):Void
	{
		var w:Dynamic = ctrl.widget;
		if (w == null) return;
		try
	...
	function openInputOverlay(ctrl:DialogueCustomControl):Void
	{
		var w:Dynamic = ctrl.widget;
		clearActiveEditWidget();
		if (stepperOverlayCtrl != null)
	...
	public function openMenu(idx:Int):Void
	{
		...
		refreshDialogueCustomControlDisplays();
	}
	...
	function rebuildDropdown():Void
	{
		...
		for (it in items)
		{
			...
			if (it.type == Widget || it.type == LabelWidget)
			{
				...
				{
					// 标签最小宽度
					var lbl = Language.get(it.labelKey, 'dialogue');
			...
			{
				var txt = Language.get('item_' + it.labelKey, 'dialogue');
				var lineH = 16;
				// 估算文本行数
				var charW = 7;
			...
			{
				var lbl = Language.get('item_' + it.labelKey, 'dialogue');
		...
		for (it in items)
		{
			...
			// === Cmd 行 ===
			var labelStr = Language.get('item_' + it.labelKey, 'dialogue');
	...
	function refreshCheckIcon(icon:FlxText, it:DialogueMenuItemDef):Void
	{
	...
	function clearDialogueDropdownRows():Void
	{
	...
	function activateRow(row:DialogueDropdownRow):Void
	{
		...
		switch (it.type)
		{
			...
			case LabelWidget:
				// 交互已由 handleDialogueCustomControlClick（自绘控件）接管：
	...
	// ============ 描述条 ============
	function updateDescBar(row:DialogueDropdownRow):Void
	{
		...
		var txt = Language.get(row.def.descKey, 'dialogue');
...
typedef DialogueMenuDef =
{
	key:String,
	isTest:Bool,
	items:Array<DialogueMenuItemDef>
};

typedef DialogueDropdownRow =
{
	type:DialogueItemType,
	labelKey:String,
	descKey:String,
	def:DialogueMenuItemDef,
	hit:FlxSprite,
...
typedef DialogueCustomControl =
{
	...
	?panelBg:FlxSprite,
	?panelRows:Array<DialogueDropdownOption>,
	?panelH:Int,
	// slider
	?track:FlxSprite,
	?fill:FlxSprite,
...
/** dropdown 展开面板的单个选项行 */
typedef DialogueDropdownOption =
{
	bg:FlxSprite,
	txt:FlxText,
	hit:FlxSprite,
